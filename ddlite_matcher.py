...
class ConcatSingleDictionaryMatch(CandidateExtractor):
  """
  Selects according to ngram-matching against a dictionary i.e. list of words.
  ...
  def _apply(self, s, idxs=None):
    ...
    # Save the end index of each match to potentially merge consecutive matches.
    matched_seqs_ordered={}
    ...
    for l in filter(lambda n : n <= L, self.ngr):
      for i in range(start, end-l+1):
        ...
        if phrase in self.dl[l]:
          matched_seqs.append(frozenset(ssidx))
          matched_seqs_ordered[ssidx[0]]=ssidx[-1]
          # yield list(ssidx), self.label
    ...
    def merging_candidates_from_dict(dict_var, k):
      if k in dict_var:
        v=dict_var[k]+1
        if v in dict_var:
          dict_var[k]=dict_var[v]
          del dict_var[v]
          merging_candidates_from_dict(dict_var, k)

    list_keys = list(matched_seqs_ordered.keys())
    for k in list_keys:
      merging_candidates_from_dict(matched_seqs_ordered, k)
